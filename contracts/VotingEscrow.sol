...
uint256 epoch;
mapping(uint256 => Point) point_history;
...
address future_admin;
  ...
  function _checkpoint(address addr,LockedBalance memory old_locked , LockedBalance memory new_locked) private {
    ///
    ...
    if (addr != address(0)) {
        ...
        if (old_locked.end > block.timestamp && old_locked.amount > 0){
            u_old.slope = old_locked.amount / SafeCast.toInt128(uinttoint(MAXTIME));
            u_old.bias = u_old.slope * SafeCast.toInt128(uinttoint(old_locked.end - block.timestamp));
        }
        if (new_locked.end > block.timestamp && new_locked.amount > 0){
            u_new.slope = new_locked.amount / SafeCast.toInt128(uinttoint(MAXTIME));
            u_new.bias = u_new.slope * SafeCast.toInt128(uinttoint(new_locked.end - block.timestamp));
        }
    ...
    for (uint16 i = 0; i < 255; i++) {
      ...
      last_point.bias -= last_point.slope * SafeCast.toInt128(uinttoint(t_i - last_checkpoint));
  ...
  function _deposit_for(address _addr ,uint256 _value, uint256 unlock_time, LockedBalance memory locked_balance,int128 _type) private {
    ...
    LockedBalance memory old_locked = locked[_addr];
  ...
  function deposit_for(address _addr,uint256 _value) public nonReentrant {
  ...
  function increase_unlock_time(uint256 _unlock_time) public nonReentrant {
    ///
  ...
  function getVotingPowerAt(address addr , uint256 _block) public view returns (uint256) {
    return balanceOf(addr,_block);
  }
  ...
  //       last_point.bias -= last_point.slope * SafeCast.toInt128(SignedSafeMath.sub(uinttoint(_t),uinttoint(last_point.ts)));
  //       if (last_point.bias < 0){
  //           last_point.bias = 0;
  //       }
  ...
  function balanceOf(address addr ) public view returns(uint256) {
    ...
    if (_epoch == 0) {
        ...
        Point memory last_point = user_point_history[addr][_epoch];
        last_point.bias -= last_point.slope * SafeCast.toInt128(uinttoint(_t - last_point.ts));
        if (last_point.bias < 0){
            last_point.bias = 0;
        }
        return inttouint(last_point.bias);
  ...
  function balanceOf(address addr,uint256 _block) public view returns(uint256) {
    ///
    ...
    upoint.bias -= upoint.slope * SafeCast.toInt128(uinttoint(block_time - upoint.ts));
    if (upoint.bias >= 0) {
  ...
  function supply_at(Point memory point, uint256 t) public view returns(uint256) {
    ...
    for (uint256 i = 0; i < 255; i++) {
      ...
      last_point.bias -= last_point.slope * SafeCast.toInt128(uinttoint(t_i - last_point.ts));
      if (t_i == t) {
          break;
      }
      last_point.slope += d_slope;
