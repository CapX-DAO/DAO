...
const listSolidityFiles = (dir) => globby(`${dir}/**/*.sol`);
const fatalErrors = [
...
export const getEtherscanPath = async (contractName) => {
...
export const verifyContract = async (contractName, address, constructorArguments, libraries) => {
...
export const runTaskWithRetry = async (task, params, times, msDelay, cleanup) => {
    let counter = times;
    await delay(msDelay);
    try {
        if (times) {
        ...
        if (fatalErrors.some((fatalError) => error.message.includes(fatalError))) {
            console.error('[ETHERSCAN][[ERROR] Fatal error detected, skip retries and resume deployment.');
            return;
        }
