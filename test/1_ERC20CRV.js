...
contract("ERC20CRV", ([deployer, receiver, sender, mintreceiver]) => {
  ...
  describe("total Supply", () => {
    it("Checking current supply at and minting", async () => {
      ...
      assert(
        result.toNumber() != result1.toNumber &&
          result1.toNumber() - result.toNumber() == 100
      );
    });
  });
  ...
  describe("tests related to transfer", () => {
    ...
    it("checks if transfer from one account to another", async () => {
      ...
      let valueToSend = 20;
      // transfer
      const result = await token.transferFrom(deployer, receiver, valueToSend, {
        from: sender,
      });
      ...
      assert(
        balance1.toNumber() - balance3.toNumber() == valueToSend &&
          balance4.toNumber() - balance2.toNumber() == valueToSend,
    ...
    it("check if approve spender to transfer tokens on behalf of a user", async () => {
      ...
      const result = await token.approve(spenderAddress, value, {
        from: deployer,
      });
  ...
  describe("Check if admin is able to set a new admin", async () => {
    it("if the user is the admin", async () => {
      ...
      assert(
        newAdmin.toString() === sender.toString(),
